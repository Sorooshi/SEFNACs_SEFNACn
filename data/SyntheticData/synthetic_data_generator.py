...
def generate_Y(N, V, K, pr_v, cardinality, features_type, V_noise1):
    ...
    if features_type == 'Q':  # quantitative
        ...
        d1, d2 = 2, 5  # 2, V + 1  # a range for subcategories

        sub_cats = []
        for v in range(V):
            sub_cats.append(np.random.randint(low=d1, high=d2))

        each_feature_sub_cat = []
        for i in range(len(sub_cats)):
            each_feature_sub_cat.append(list(np.arange(1, sub_cats[i] + 1)))

        nc = len(cardinality)  # Number of Categories
        all_cf = [i for i in itertools.product(*each_feature_sub_cat)]  # All Possible Categorical Features
        if sum(sub_cats) >= nc:
            community_to_feature_matrix = random.sample(all_cf, nc)
        else:
            community_to_feature_matrix = random.choices(all_cf, k=nc)

        features_matrix = np.zeros((nc, V))
        for i in range(len(community_to_feature_matrix)):
            for j in range(len(community_to_feature_matrix[i])):
                features_matrix[i, j] = community_to_feature_matrix[i][j]
        ...
        for k in range(K):
            for i in range(interval, cardinality[k] + interval):
                if random.random() > pr_v:
                    features = [int(f) for f in features_matrix[k, :]]
                    noisey_features = []
                    for j in range(len(features)):
                        noisey_features.append(
                            np.random.choice(list(set(range(1, sub_cats[j] + 1)).difference([features[j]])), 1))
                    Y[i, :] = noisey_features
                else:
                    Y[i, :] = features_matrix[k, :]
            interval += cardinality[k]
        ...
        d1, d2 = -1, 1  # a range for mean
        d1_, d2_ = 0.025 * (d2 - d1), 0.05 * (d2 - d1)  # a range for covatiance matrix

        Y = np.zeros([N, V])  # emptity entity-to-feature matrix
        interval = 0
        for k in range(K):
        ...
        # Categorical Section:
        # As it is mentioned in the paper (just to reduce the space complexity)
        d1, d2 = 2, 5  # (10 for MM)2, Vc + 1  # a range for subcategories 
            ...
            for i in range(interval, cardinality[k] + interval):
                if random.random() > pr_v:
                    ...
                    for j in range(len(features)):
                        noisey_features.append(
                            np.random.choice(list(set(range(1, sub_cats[j] + 1)).difference([features[j]])), 1))
                    Y[i, Vq:] = noisey_features
                else:
                    Y[i, Vq:] = features_matrix[k, :]
            interval += cardinality[k]
